...
-- In this challenge we use the operators "OR" "AND". The "AND" operator has precence over the "OR",
-- if we have both of the them in the same query, but we can always use parenthses to change the order of evaluation.
-- When using the "OR" operator you are saying one of those two conditions has to be true, when using "AND" all those 
