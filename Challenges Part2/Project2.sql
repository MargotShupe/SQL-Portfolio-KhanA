...
-- I picked Solar system objects by size
...
SELECT body, SUM(volume) FROM solar_system_objects GROUP BY body;
SELECT body, AVG(volume) AS avg_volume FROM solar_system_objects GROUP BY body
HAVING avg_volume > 50000;
SELECT COUNT (*),
    CASE
        WHEN volume > 1100000000 THEN "HYPER STAR"
        WHEN volume > 1400000 THEN "MEGA STAR"
        WHEN volume > 820000 THEN "SUPER STAR"
        ELSE "STAR"
    END AS "star_category"
FROM solar_system_objects
GROUP BY "star_category"
SELECT * FROM solar_system_objects WHERE ( volume > 1000 AND mean_radius_rel > 3.00)
OR ( mean_radius > 1000);