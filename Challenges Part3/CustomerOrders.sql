...
-- every customer followed by the item and price of orders they've made. Use a LEFT OUTER JOIN
...
-- email, and total amount of money they've spent on orders. Sort the rows according to the total money spent, 
...
/* An inner join only creates rows if there are matching records in the two tables.
The left outer join is telling us to retain every row from the left table and the and the outer
it's saying to retain the rows even if there is a not matching in the right table.*/